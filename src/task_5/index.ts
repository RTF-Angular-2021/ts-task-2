...
class BankTerminal {
	...
	public takeUsersMoney(moneyUnits: Array<IMoneyUnit>): boolean {
		if (typeof this._authorizedUser !== "undefined"
			&& moneyUnits.length > 0) {
			this._moneyRepository.takeMoney(moneyUnits);

			let cardId = this._authorizedUser.cards[0].id;
			let count: number = 0;
			for (let moneyUnit of moneyUnits) {
				count += Number(moneyUnit.moneyInfo.denomination) * moneyUnit.count;
			}
			return this._bankOffice.addMoneyToCartById(cardId, count);
		}
		return false;
	}
